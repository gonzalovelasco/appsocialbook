...
class Data {
  static clear() {
    ...
    BookUser.collection.drop();
  }

  static generateUser() {
    let user = new User({name: 'Foobar', username: 'foobar', password: 'password'});
    user.save((err) => {
      if(err){
        console.log(err);
      }
  ...
  for (var i = 0; i < data.length; i++) {
    objects.push(new Book({author: data[i].author, genre: data[i].language ,title:data[i].title, img: "http://via.placeholder.com/150x200" }));
  }

    Book.insertMany(objects).then((docs) => {
    })
    .catch((err) => {
        console.log(err);
    });
  ...
  static generateBookRelation() {

    User.findOne({ 'name': 'Foobar' }, (err, user)  => {
      if(err){
        console.log(err);
      }
